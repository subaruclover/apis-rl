#!/usr/bin/env python
"""
DQN testing, single run, house E002
...
# Testing function
def test(RL):
    print("House E002, testing start")
    total_steps = 0
    steps = []
    episodes = []
    reward_list = []
    ...
    for i_episode in range(EPI):
        ...
        while day < N_DAY:
            ...
            observation_, reward, info = env.step2_time(action_request, action_accept, house_id)

            # actions_space = np.around(np.linspace(0.3, 0.9, 7).tolist(), 1)
            actions_space = np.linspace(0.2, 0.9, 8).tolist()
            print("House E002, Scenario file updated with act_req {}, {} and act_acc {}".format(
...
saver = tf.train.import_meta_graph('model/E002/E002_model_prio.meta')
with tf.Session() as sess:
    saver.restore(sess, 'model/E002/E002_model_prio')
...
with open("saved/prio_memo_e002_Feb_test.data", "wb") as fp:
    pickle.dump(prio_memory, fp)
# save reward to json file
with open("saved/prio_reward_e002_Feb_test.data", "wb") as fp:
