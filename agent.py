...
class House():
    ...
    def time_of_day(self, df):
        # get time info
        loop = np.linspace(0, 2880-1, 2880, dtype=int)
        # print(type(loop))
        loop_list = []

        for i in range(int(len(df) / 2880)):
            loop_list = np.append(loop_list, loop)

        df["x"] = loop_list

        # Normalize x values to match with the 0-2π cycle
        df["x_norm"] = 2 * math.pi * df["x"] / 2880

        df["cos_x"] = np.cos(df["x_norm"])
        df["sin_x"] = np.sin(df["x_norm"])

        time_cos = df[["cos_x"]].values
        time_sin = df[["sin_x"]].values

        return time_cos, time_sin

    def step1_2(self, action_request, action_accept, house_id):
